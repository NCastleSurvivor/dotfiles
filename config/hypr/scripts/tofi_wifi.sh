...
show_wifi_menu() {
	...
	wifi_list=$(nmcli -t -f SSID,BSSID,IN-USE,SIGNAL dev wifi | awk -F: '{print $1}')
        ...
        selected_ssid=$(echo -e "Refish\nAdd\n$wifi_list" | tofi --prompt="Select Wi-Fi network: " )
        ...
        if [ "$selected_ssid" == "Add" ]; then
            local new_ssid=$(tofi --prompt="Enter Wi-Fi name (SSID): ")
            local new_password=$(tofi --prompt="Enter password for $new_ssid: " --input-type=password)
        ...
        password=$(tofi --prompt="Enter password for $selected_ssid: " --input-type=password)
